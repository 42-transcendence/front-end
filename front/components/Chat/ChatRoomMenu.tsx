...
export function ChatRoomMenu({ isAdmin }: { isAdmin: boolean }) {
    return (
        <div className="flex flex-col items-center text-base font-bold text-gray-100/80">
            ...
            {isAdmin && <MenuItem className="active:bg-secondary/80">정지 유저 관리</MenuItem>}
            {isAdmin && <MenuItem className="active:bg-secondary/80">소유권 양도</MenuItem>}
            {isAdmin && <MenuItem className="active:bg-red-500/80">채팅방 삭제</MenuItem>}
